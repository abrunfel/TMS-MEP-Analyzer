...
MT_c = MT_c';
bvup_mean = nanmean(MT_c(upTrials(1:6)));
bvup_std = nanstd(MT_c(upTrials(1:6)));
MT_up_st = (MT_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(MT_c(downTrials(1:6)));
bvdown_std = nanstd(MT_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
ide_c = ide_c';
bvup_mean = nanmean(ide_c(upTrials(1:6)));
bvup_std = nanstd(ide_c(upTrials(1:6)));
ide_up_st = (ide_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(ide_c(downTrials(1:6)));
bvdown_std = nanstd(ide_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
rmse_c = rmse_c';
bvup_mean = nanmean(rmse_c(upTrials(1:6)));
bvup_std = nanstd(rmse_c(upTrials(1:6)));
rmse_up_st = (rmse_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(rmse_c(downTrials(1:6)));
bvdown_std = nanstd(rmse_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
EPE_c = EPE_c';
bvup_mean = nanmean(EPE_c(upTrials(1:6)));
bvup_std = nanstd(EPE_c(upTrials(1:6)));
EPE_up_st = (EPE_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(EPE_c(downTrials(1:6)));
bvdown_std = nanstd(EPE_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
EP_X_c = EP_X_c';
bvup_mean = nanmean(EP_X_c(upTrials(1:6)));
bvup_std = nanstd(EP_X_c(upTrials(1:6)));
EP_X_up_st = (EP_X_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(EP_X_c(downTrials(1:6)));
bvdown_std = nanstd(EP_X_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
EP_Y_c = EP_Y_c';
bvup_mean = nanmean(EP_Y_c(upTrials(1:6)));
bvup_std = nanstd(EP_Y_c(upTrials(1:6)));
EP_Y_up_st = (EP_Y_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(EP_Y_c(downTrials(1:6)));
bvdown_std = nanstd(EP_Y_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
mov_int_c = mov_int_c';
bvup_mean = nanmean(mov_int_c(upTrials(1:6)));
bvup_std = nanstd(mov_int_c(upTrials(1:6)));
mov_int_up_st = (mov_int_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(mov_int_c(downTrials(1:6)));
bvdown_std = nanstd(mov_int_c(downTrials(1:6)));
...
bvup_mean = nanmean(norm_jerk_c(upTrials(1:6)));
bvup_std = nanstd(norm_jerk_c(upTrials(1:6)));
norm_jerk_up_st = (norm_jerk_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(norm_jerk_c(downTrials(1:6)));
bvdown_std = nanstd(norm_jerk_c(downTrials(1:6)));
...
% transpose and calculate standardized variable
velPeak_c = velPeak_c';
bvup_mean = nanmean(velPeak_c(upTrials(1:6)));
bvup_std = nanstd(velPeak_c(upTrials(1:6)));
velPeak_up_st = (velPeak_c(upTrials) - bvup_mean)/bvup_std;

bvdown_mean = nanmean(velPeak_c(downTrials(1:6)));
bvdown_std = nanstd(velPeak_c(downTrials(1:6)));
...
save([fname '_postStep2_rh' '.mat'],'sortData','downTrials','upTrials','fname','onset','offset','wrong_trial','groupID','EP_X', 'EP_X_c', 'EP_X_down_st', 'EP_X_up_st',...
